...
def draw(canvas):
    ...
    if(ball_pos[0] <= (BALL_RADIUS + PAD_WIDTH)): #left gutter
        if(ball_pos[1] - BALL_RADIUS >= paddle1_pos and ball_pos[1] + BALL_RADIUS <= paddle1_pos + PAD_HEIGHT):  #paddle
    ...
    if(ball_pos[0] >= WIDTH - 1 - PAD_WIDTH - BALL_RADIUS): #right gutter
        if(ball_pos[1] - BALL_RADIUS >= paddle2_pos and ball_pos[1] + BALL_RADIUS <= paddle2_pos + PAD_HEIGHT): #paddle
            ball_vel[0] = -ball_vel[0]
            ball_vel[0] += ball_vel[0]*0.1
            ball_vel[1] += ball_vel[1]*0.1
        else:
